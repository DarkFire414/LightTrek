...
/* otras variables */
static double  r_arr[2001]; /*Arreglo a retornar en python*/
static double  fsph_arr[2001];
static double  fcypl_arr[2001];
static double  fcpla_arr[2001];
...
//La variable nst debe ser declarada como un arreglo de char para que funcione strcpy
char nst[40];
...
void saveToFile(char* name) {
	...
	for (ir=0; ir<=NR; ir++) 
	{
		/* 
			...
			Dividiendo cada concentración en el paquete por el coeficiente de absorción produce el ritmo de 
...
int initSim(double Nphotonsi, double muai, double musi, double gi, double nti, int sourceti) {
	...
	*****/
	...
	Nphotons = Nphotonsi;   // Número de fotones a lanzar N= 
	mua = muai;             // [Coef. de absorcion]     æa = 
	mus = musi;             // [Coef. de esparcimiento] æs = 
	g = gi;                 // [Coef. de anisotropia]    g = 
	nt = nti;               // [Indice de refracción]   nt = 
	/*
	    Tipo de Fuente:
	        (1)Puntual isotrópica
	        (2)Colimada
	        (3)Haz finito
	*/
	sourcet = sourceti;
...
//	Nphotons    = 20000; 	/* Colocar el número de fotones en la simulación a lanzarse */
	...
	*****/
	...
	//RandomGen(0,random(100),NULL); // Pasa como segundo argumento un número aleatorio del 1 al 100
	RandomGen(0,rand()%100,NULL); 
	//Posición radial
	for (ir=0; ir<=NR; ir++)
	{
	   Csph[ir] = 0;
...
//		z = 0;
		...
		*******/
		...
		/* MODIFICACION */
		// Cambio de la funcion cprintf pot printf
		// Se agrego un % al % (%%)
		gotoxy(12,15); printf("%3.2f %%", 100*(i_photon / Nphotons) );
